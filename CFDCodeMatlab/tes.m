...
for j=(n_h2+n_w2)+1:(n_h2+n_w2+n_h1)+1%Ghost point for right-up vertical wall
    v(j,n_w1+2)=2*v_eu-v(j,n_w1+2-1);
end
...
for i=n_w1+1:n_w1+n_l+1%Ghost point for up and down horizental wall
   ...
   u((n_h2+n_w2)+2,i)=2*u_n-u((n_h2+n_w2)+2-1,i);%up
end
...
for I=1:n_w1+n_l+2
        for J=1:n_h2+n_w2+n_h1+2 %M=5
            if (I>=n_w1+2) && ((J<=n_h2+2-1) || (J>=n_h2+n_w2+2))
                mu=mu_inf;
                pp0=0;
             else
                mu=mu0;
...
for I=n_w1+2:n_w1+n_l+2-1%ghost point for up and down horizental wall
    ...
    p((n_h2+n_w2)+2,I)=p((n_h2+n_w2)+2-1,I);%up
end
...
for o=1:m%Tol>tolorance
    ...
    for j=1:n_h2+1%Ghost point for right-bottom wall
        vv(j,n_w1+2)=2*v_ed-vv(j,n_w1+2-1);
    end

    for j=(n_h2+n_w2)+1:(n_h2+n_w2+n_h1)+1%Ghost point for right-up wall
        vv(j,n_w1+2)=2*v_eu-vv(j,n_w1+2-1);
    end
    ...
    for I=2:n_w1+2-1%ghost point for inlet1 and inlet2
        pp(1,I)=0;%pp(1+1,I);%inlet1
        pp((n_h2+n_w2+n_h1)+2,I)=0;%pp((n_h2+n_w2+n_h1)+2-1,I);%inlet2
    end

    for I=n_w1+2:n_w1+n_l+2-1%ghost point for up and down horizental wall
        pp(n_h2+2-1,I)=pp(n_h2+2,I);%down
        pp((n_h2+n_w2)+2,I)=pp((n_h2+n_w2)+2-1,I);%up
    end
